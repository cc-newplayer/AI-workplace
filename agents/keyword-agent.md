...
### STEP K0.5：当前策略过滤器【策略V2 — 2026-04-10起，可回退】

> 回退方法：删除本步骤（K0.5）即可恢复V1策略

**当前只接受以下两类话题，其他一律不推荐：**

✅ **允许 — 职业冲击类**：某职业/行业被AI替代的现状、真实裁员案例、哪些技能还值钱、副业受冲击等
✅ **允许 — 实用工具教程类**：普通人能直接上手的AI工具操作教程（ChatGPT/Claude/Kimi/豆包/可灵等），场景越具体越好

❌ **暂停 — 架构/协议/原理类**：Agent架构、MCP协议、LangGraph、RAG原理、KV Cache等开发者内容
❌ **暂停 — 深度技术评述类**：某技术为何胜出、某框架深度对比等风格B内容

若 `TOPIC: auto` 模式扫描到的热点属于暂停类别，跳过该热点，继续扫描直到找到符合条件的话题。若找不到，输出说明并停止。
