...
**【策略V2 — 2026-04-10起，可回退】当前策略约束：**
- 只允许风格 A 和风格 C，**风格 B 暂停使用**
- 若关键词触发风格B，改判为风格C（大众科普角度重写），除非关键词明确是操作教程（则判风格A）
- 回退方法：删除本段"策略约束"即可恢复V1行为

**连续 C 保护**：读取 `article-index.md` 已发布文章总览表，查看**最后一行**的风格列，若上一篇为风格 C，跳为风格 A。
...
1. **具体案例（2-3个）**：每个核心观点配 1 个普通人能感受到的真实案例，必须含具体数字/公司名/时间（如"Klarna 2024年用 AI 替掉 700 个客服岗位"），禁止"提升效率""更快更好"等无数字表述

2. **观点逆转（1处）**：提出 1 个与普遍认知不同的洞察（如"很多人以为AI只影响低技能岗位，实际上白领受冲击更早"），让文章有记忆点

3. **受众影响段（100-200字）**：明确说"这对你意味着什么"，要具体到职业或场景（如"如果你是做文案的……"），禁止笼统的"提升效率"

4. **真实反思结论（100-200字）**：诚实讨论局限或不确定性，禁止"这是很有前景的技术""相信未来会更好"等空洞展望

5. **小标题结构自然化**：
   - H2 总数控制在 4-6 个，不超过 8 个
   - 禁止教科书式序列（"什么是 → 如何工作 → 优缺点 → 未来展望 → 总结"）
   - 禁止 AI 特征标题：「未来会怎样」「你应该怎么做」「路径 1/2/3」「总之…」
   - 标题紧随内容自然转折，不是每 200 字强行插入
   - 标题用具体内容描述（「真实公司已经在砍这些岗位」而非「影响分析」）
...
观点逆转：第 X 段（一句话概括）
受众影响段：第 X 段（N 字）
真实反思结论：第 X 段（N 字），无空洞展望 ✓/✗
小标题数量：共 N 个 H2（目标 4-6 个）✓/✗
```
